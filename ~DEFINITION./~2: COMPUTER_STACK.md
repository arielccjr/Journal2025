...
~DATABASE FUNDAMENTALS:
    : DATABASE THEORY: Ensures efficient, consistent, and scalable database designs.
        Learn Relational theory, Normalization, and database architecture.
            Business Rules:
            ...
            Relational DBMS: A type of DBMS that organizes data into tables with rows and columns.
                ...
                Attributes: Properties of entities. They have a domain, which is the set of valid values an attribute can assume 
                    ...
                    Non-Key: depend only on the primary key
            Keys:
                Primary Key is a unique identifier for each record in a table (e.g., CustomerID). 
                Technical Key is an artificial identifier created by the database designer when no natural unique attribute exists 
                Composite Key (or Concatenated Key) is a primary key that consists of two or more attributes 
                Foreign Key is an attribute in one table that references the primary key in another table to establish a relationship 
    ...
    : NORMALIZATION: A process for organizing data to reduce redundancy and ensure consistency.
        Apply 1NF, 2NF, and 3NF sequentially to refine tables.
